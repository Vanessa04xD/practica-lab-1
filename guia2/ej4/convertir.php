        ...
        <?php if ($_SERVER['REQUEST_METHOD'] === 'POST'): ?>
            <div class="mt-4">
                <?php
                    ...
                    try {
                        // Validar caracteres del número según la base de origen
                        if (!preg_match('/^[0-'.(base_convert('z', 36, $base_from)-1).']+$/i', $number)) {
                            throw new Exception("El número ingresado contiene caracteres no válidos para la base $base_from.");
                        }
                        ...
                        echo "</div>";
                        echo "<div class='alert alert-success'>El número <strong>$number</strong> en base <strong>$base_from</strong> convertido a base <strong>$base_to</strong> es: <strong>$converted</strong>.</div>";
                    } catch (Exception $e) {
